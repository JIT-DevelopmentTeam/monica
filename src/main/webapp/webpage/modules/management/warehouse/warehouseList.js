...
$(document).ready(function() {
	$('#warehouseTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                       ...
                       view(row.id);
                        ...
                        jp.confirm('确认要删除该仓库管理表记录吗？', function(){
                       	jp.loading();
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
				   ...
				   <c:choose>
					   <c:when test="${fns:hasPermission('warehouse:warehouse:edit')}">
					      return "<a href='javascript:edit(\""+row.id+"\")'>"+value+"</a>";
				      </c:when>
					  <c:when test="${fns:hasPermission('warehouse:warehouse:view')}">
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  jp.downloadFile('${ctx}/warehouse/warehouse/import/template');
				  },
			    btn2: function(index, layero){
				        var iframeWin = layero.find('iframe')[0]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
						iframeWin.contentWindow.importExcel('${ctx}/warehouse/warehouse/import', function (data) {
							if(data.success){
								jp.success(data.msg);
								refresh();
							}else{
	 ...
	 $("#export").click(function(){//导出Excel文件
			...
			jp.downloadFile('${ctx}/warehouse/warehouse/export?'+values);
	  })
  ...
  function refresh(){
   ...
   function add(){
	  jp.openSaveDialog('新增仓库管理表', "${ctx}/warehouse/warehouse/form",'800px', '500px');
  }
   ...
   function edit(id){//没有权限时，不显示确定按钮
	...
	jp.openSaveDialog('编辑仓库管理表', "${ctx}/warehouse/warehouse/form?id=" + id, '800px', '500px');
  }
 ...
 function view(id){//没有权限时，不显示确定按钮
      if(id == undefined){
        ...
        jp.openViewDialog('查看仓库管理表', "${ctx}/warehouse/warehouse/form?id=" + id, '800px', '500px');
 }
